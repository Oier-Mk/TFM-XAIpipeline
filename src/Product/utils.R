...
log <- function(input, output, rules) {
  ...
  combined_list <- list(Sys.time(), input, output, rules)
  
  # Convert the combined list to JSON format
  new_json_string <- toJSON(combined_list, pretty = TRUE)
  
  # Check if the file exists and append if it does
  if (file.exists("log.json")) {
    write(new_json_string, "log.json", append = TRUE)  # Append to existing file
  } else {
    write(new_json_string, "log.json")  # Create a new file
  }
}
